# Repack supervisor APK with fresh web assets, permissions, and Titan icons.
...
$fallbackBase = Join-Path (Split-Path $root -Parent) "TitanSupervisor-v1.1.4.apk"
if ((Test-Path $fallbackBase) -and (Test-Path $srcApk)) {
  $curLen = (Get-Item $srcApk).Length
  if ($curLen -lt 4200000) {
    Write-Host "Restoring security.apk from TitanSupervisor-v1.1.4.apk (safe base)..." -ForegroundColor Yellow
...
if (-not (Test-Path $assetsSrc)) {
  throw "Web assets not found. Run: cd mobile-supervisor; npm run cap:sync"
}

$pkg = Get-Content (Join-Path (Split-Path $root -Parent) "package.json") -Raw | ConvertFrom-Json
$distApk = Join-Path (Split-Path $root -Parent) "TitanSupervisor-v$($pkg.version).apk"
