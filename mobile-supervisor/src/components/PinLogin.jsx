...
export default function PinLogin({
  ...
  const submitPin = async (nextPin) => {
    ...
    try {
      ...
      if (res.ok && json.supervisor) {
        playLoginSuccess();
        onLogin(json.supervisor, { mustChangePin: !!json.mustChangePin, currentPin: code });
        return;
      }

      playPinError();
      setError(json.error || 'Invalid PIN — check your email for your supervisor login code');
      setShake(true);
      setPin('');
      setTimeout(() => setShake(false), 500);
    } catch {
      playPinError();
      setError('Cannot reach Titan server — check your internet connection');
      setPin('');
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className={`pin-login ${submitting ? 'pin-login-busy' : ''}`}>
      ...
      <div className="pin-login-intro">
        <h2 className="pin-login-heading">Supervisor Sign In</h2>
        <p className="pin-login-sub">
          Enter the 6-digit PIN emailed when you were registered as a supervisor.
        </p>
      ...
      <div className="pin-login-keyboard">
        ...
        <button type="button" className="pin-submit-btn" disabled={!canSubmit} onClick={() => submitPin()}>
          {submitting ? (
            <>
