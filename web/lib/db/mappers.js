...
export function guardToRow(g, tenantId) {
  ...
  return {
    ...
    supervisor_id: g.supervisorId || null,
...
export function checkpointToRow(cp, tenantId) {
  return {
    ...
    coords_x: cp.coords?.x != null ? Math.round(cp.coords.x) : null,
    coords_y: cp.coords?.y != null ? Math.round(cp.coords.y) : null,
...
export function swapToRow(s, tenantId) {
  return {
    ...
    tenant_id: tenantId,
    shift_id: s.shiftId,
    requester_guard_id: s.requestingGuardId || s.requesterGuardId,
...
export function rowToSwap(r) {
  return {
    ...
    requestingGuardId: r.requester_guard_id,
